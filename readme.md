...
7. Сжатие данных при передаче тесктовых сообщений (Sec-WebSocket-Extensions: permessage-deflate)
8. Определить библиотеку golang для реализации websocket
...
- [gorilla/websocket](https://github.com/gorilla/websocket/). Старый добрый стандарт. Огромное комьюнити, но API немного устарел (нет встроенных контекстов) и проект переведен в архивный статус (хотя критические правки вносятся).
- [gobwas/ws](https://github.com/gobwas/ws). Экстремальная производительность, низкий уровень, минимум аллокаций. Подходит, если у вас миллион соединений и нужно экономить каждый байт RAM.
- [coder/websocket](https://github.com/coder/websocket). Золотая середина. Идиоматичный Go-код, поддержка HTTP/2 и отличная работа с контекстами. Идеальный выбор для новых проектов.

Сравнение трех самых популярных библиотек для WebSocket в Go с фокусом на критически важную поддержку permessage-deflate (RFC 7692). Это ключевой параметр для экономии трафика, но он сильно нагружает процессор и память.

> [!NOTE]
> coder/websocket - это то же самое, что и nhooyr/websocket, но под официальным крылом компании Coder.
> В 2024 году автор (nhooyr) передал проект на обслуживание Coder, чтобы обеспечить его долгосрочное развитие и безопасность

| Характеристика | Gorilla | Coder (ex-nhooyr) | Gobwas |
| :--- | :----: | ---: | ---: |
| Стиль API | Традиционный<br>(callbacks) | Современный<br>(Context) | Низкоуровневый<br>(Zero-copy) |
| Производительность | Средняя | Высокая | Максимальная<br>(для High-load) |
| Поддержка Context | Нет | Да<br>(нативная) | Нет<br>(нужно оборачивать) |
| Статус проекта | В архиве / Стабилен | Активно поддерживается | Активен |
| Сложность освоения | Низкая | Низкая | Высокая |
| Поддержка permessage-deflate | Да<br>(частичная) | Да<br>(полная) | Опционально |

Особенности сжатия

- [gorilla/websocket](https://github.com/gorilla/websocket/). Реализовано без «context takeover». Это означает, что словарь сжатия сбрасывается для каждого сообщения, что менее эффективно для маленьких сообщений, но экономит RAM.
- [gobwas/ws](https://github.com/gobwas/ws). Заявляет о полной поддержке расширения. Использует современные подходы для минимизации аллокаций даже при сжатии.
- [coder/websocket](https://github.com/coder/websocket). В ядре библиотеки сжатия нет. Для этого используется отдельный пакет ws/wsflate, который предоставляет низкоуровневые обертки над compress/flate.

Для нашего примера достаточно библиотеки [gorilla/websocket](https://github.com/gorilla/websocket/)

# Client

```
$ wget -qO ~/.local/bin/websocat https://github.com/vi/websocat/releases/latest/download/websocat.x86_64-unknown-linux-musl
```
...
**Шаг 4**. Без установленного дедлайна WriteMessage может зависнуть навсегда, если клиент перестал подтверждать получение пакетов, а системные буферы переполнились. Разработчики библиотеки [github.com/gorilla/websocket](https://github.com/gorilla/websocket) настоятельно рекомендуют всегда устанавливать дедлайн перед каждой операцией записи, чтобы избежать утечки горутин.
...
**Шаг 6** Мониторинг соединений на уровне Nginx. Можно смотреть количество активных соединений через модуль stub_status.
...
**Шаг 7** Балансировка WebSocket сложнее обычной, так как это stateful протокол: соединение держится долго, и разорвать его — значит заставить клиента переподключаться.
...
**Шаг 9**. Данные передаются как текст в кодировке UTF-8 (websocket.TextMessage = 1). В пакете [github.com/gorilla/websocket](https://github.com/gorilla/websocket) определено несколько типов сообщений, которые соответствуют спецификации протокола RFC 6455.
...
Текстовые данные (JSON, логи) могут сжиматься в 5–10 раз. Если вы отправляете 100 КБ текста, в сжатом виде это может стать 15 КБ, что легко пролетит через стандартные буферы.

> [!NOTE]
> Нагрузка на CPU: Сжатие каждого сообщения потребляет ресурсы процессора на сервере.

По умолчанию в Gorilla сжатие выключено. Чтобы оно заработало, нужно обновить Upgrader
...
Уровни сжатия в Gorilla WebSocket (как и в стандартном библиотечном пакете compress/flate в Go) варьируются от -2 до 9.

- **-2** (HuffmanOnly): Сжатие только методом Хаффмана (очень быстро, эффективно для специфических данных).
- **-1** (DefaultCompression). Баланс между скоростью и качеством. Обычно это уровень 6.
- **0** (NoCompression). Сжатие отключено.
- **1** (BestSpeed). Самый быстрый режим. Минимум нагрузки на CPU, но размер файла уменьшится не сильно.
- **2 - 8** Промежуточные уровни.
- **9** (BestCompression). Максимальное сжатие. Текст сожмется очень сильно, но это сильно нагрузит процессор сервера при большом количестве клиентов.

> [!NOTE]
> - **Для JSON и текста**: Уровень 1 или 2 обычно дает 70-80% того же результата, что и уровень 9, но работает в разы быстрее.
> - **Если клиентов много**: Ставьте 1. Это сэкономит трафик, не «убивая» CPU сервера.
> - **Если важна экономия трафика (мобильные сети)**: Ставьте 6 (дефолт). Поднимать до 9 редко имеет смысл, так как разница в байтах будет копеечная, а задержка (Latency) вырастет.

```
$ websocat -vv -H"Sec-WebSocket-Extensions: permessage-deflate; client_max_window_bits" ws://localhost:8000/ws-notifications
...
[INFO  websocat::ws_client_peer] Connected to ws, response headers: Headers { Upgrade: websocket
, Connection: Upgrade
, Sec-WebSocket-Accept: cqL5wpt0GS0FoypnFCBDJN+gSt0=
